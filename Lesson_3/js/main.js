...
function userInfo () {
	return this.registered ? `Дата регистрации: ${this.data}` :
	`Незарегестрированный пользователь: ${this.name}`
}
...
function getCurrentPostComments ( postId ) {
	...
	for (var comment in comments) {
		for (var key in comments[comment]) {
			if (comments[comment][key] === postId) {
				obj.name = users[comments[comment].author].name
